...
const ENDORSEMENTS_PATH = path.join(__dirname, "../src/data/endorsements.json");

// Load endorsements (generated by the earlier build step) and group their slugs
// by linked Experience name (Calendar Title), so every agenda event sharing
// that title — e.g. recurring ClawCamp sessions — carries the same endorsement
// slugs in its endorsementIds array.
function loadEndorsementSlugsByExperienceName() {
  const map = new Map();
  if (!fs.existsSync(ENDORSEMENTS_PATH)) return map;
  try {
    const data = JSON.parse(fs.readFileSync(ENDORSEMENTS_PATH, "utf8"));
    if (!Array.isArray(data)) return map;
    for (const e of data) {
      if (!e?.experienceName || !e?.slug) continue;
      const slugs = map.get(e.experienceName) ?? [];
      slugs.push(e.slug);
      map.set(e.experienceName, slugs);
    }
  } catch (err) {
    console.warn(`  ⚠ Failed to load endorsements for agenda linking: ${err.message}`);
  }
  return map;
}
...
async function main() {
  ...
  const endorsementSlugsByExperienceName = loadEndorsementSlugsByExperienceName();
  ...
  const events = await Promise.all(records.map(async (r) => {
    ...
    return {
      ...
      endorsementIds: title ? endorsementSlugsByExperienceName.get(title) : undefined,
    };
  }));
